...
SemanticAnalyzer.prototype.rules = {
  ...
  'number literal': function(semantics) {
    if (this.value.match(/\.|e/)) {
      this.meta.type = new Type('float='+this.value)
    } else {
  ...
  'assignment expression': function(semantics) {
    ...
    if (!type2.canPromoteTo(type1)) {
      var line = this.children[1].line
      var column = this.children[1].column
      if (type2.canCastTo(type1)) {
        var message = 'Cannot assign `' + type2 + '` into `' + type1 + '` without data loss.'
        semantics.error(message, line, column)
        semantics.note('Use a typecast to perform the conversion explicity.', this.children[2].line, this.children[2].column)
      } else {
