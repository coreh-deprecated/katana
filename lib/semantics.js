...
SemanticAnalyzer.prototype.checkIfLogical = function(symbol, context) {
  if (!symbol.meta.type.isLogical()) {
    var message = 'Cannot perform ' + context + ' with operand of type `' + symbol.meta.type + '`.'
    this.error(message, symbol.line, symbol.column)
  }
}

SemanticAnalyzer.prototype.checkIfNumeric = function(symbol, context) {
  if (!symbol.meta.type.isNumeric()) {
    var message = 'Cannot perform ' + context + ' with operand of type `' + symbol.meta.type + '`.'
    this.error(message, symbol.line, symbol.column)
  }
}

SemanticAnalyzer.prototype.checkIfIntegral = function(symbol, context) {
  if (!symbol.meta.type.isIntegral()) {
    var message = 'Cannot perform ' + context + ' with operand of type `' + symbol.meta.type + '`.'
    this.error(message, symbol.line, symbol.column)
  }
}
...
SemanticAnalyzer.prototype.rules = {
  ...
  'logical expression': function(semantics) {    
    var operation
    switch (this.children[1].value) {
      case '^^': operation = 'logical \'xor\''; break
      case '&&': operation = 'logical \'and\''; break
      case '||': operation = 'logical \'or\''; break
    }
    semantics.checkIfLogical(this.children[0], operation)
    semantics.checkIfLogical(this.children[2], operation)
  ...
  'bitwise expression': function(semantics) {    
    var operation
    switch (this.children[1].value) {
      case '^': operation = 'bitwise \'xor\''; break
      case '&': operation = 'bitwise \'and\''; break
      case '|': operation = 'bitwise \'or\''; break
    }
    semantics.checkIfIntegral(this.children[0], operation)
    semantics.checkIfIntegral(this.children[2], operation)
    this.meta.type = semantics.checkTypeInteraction(this.children[0], this.children[2], this.children[1])
  },
  ...
  'relational expression': function(semantics) {    
    ...
    semantics.checkIfNumeric(this.children[0], operation)
    semantics.checkIfNumeric(this.children[2], operation)
    semantics.checkTypeInteraction(this.children[0], this.children[2], this.children[1])
    this.meta.type = new Type('bool')
  },

  'bitshift expression': function(semantics) {    
    var operation = 'bitshift'
    semantics.checkIfIntegral(this.children[0], operation)
    semantics.checkIfIntegral(this.children[2], operation)
  ...
  'addition expression': function(semantics) {    
    if (this.children[0].meta.type.is('pointer')) {
      semantics.checkIfIntegral(this.children[2], 'pointer arithmetic')
      this.meta.type = this.children[0].meta.type
    } else {
      var operation
      switch (this.children[1].value) {
