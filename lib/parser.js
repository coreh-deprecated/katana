...
var Parser = function(rewriterOutput) {
  ...
  this.semantics = new SemanticAnalyzer(this.errors)
...
Parser.prototype = {
  ...
  ExportStatement: function() {
    ...
    return new this.Symbol('export statement', { children: [this.DeclarationStatement()] })
  },
  ...
  DeclarationStatement: function() {
    var type = this.Type()
    var declarationList = this.DeclarationList(type.meta.type)
  ...
  DeclarationList: function(type) {
    var declarations = []
    for (;;) {
      declarations.push(this.Declaration(type))
      if (!this.eat('comma')) {
        break
      }
    }
  ...
  Declaration: function(type) {
    ...
    if (assignmentOperator = this.eat('assignment operator', '=')) {
      ...
      return new this.Symbol('declaration', { meta: { type: type }, children: [identifier, assignmentOperator, expr] })
    } else {
      return new this.Symbol('declaration', { meta: { type: type }, children: [identifier] })
    }
  },
  ...
  ArrayLiteral: function() {
    ...
    if (this.eat('square bracket', ']')) {
      return new this.Symbol('array literal', { children: [] })
    } else {
      var contents = []
      for (;;) {
        try {
          ...
          if (this.eat('square bracket', ']')) {
            return new this.Symbol('array literal', { children: contents })
          } else {
            this.automaticComma()
          }
        } catch (err) {
    ...
    if (this.eat('curly bracket', '}')) {
      ...
      for (;;) {
        try {
          ...
          if (this.eat('curly bracket', '}')) {
            return new this.Symbol('object literal', { children: contents })
          } else {
            this.automaticComma()
          }
        } catch (err) {
  ...
  FunctionLiteral: function() {
    if (this.eat('keyword', '\\take')) {
      return new this.Symbol('function literal', { children: [this.DeclarationList(), this.Block()] })
    } else if (this.eat('keyword', '\\do')) {
      return new this.Symbol('function literal', { children: [this.Block()] })
    }
  }
}
