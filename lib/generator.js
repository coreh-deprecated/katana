...
var runtimeHeader = fs.readFileSync(__dirname + '/../runtime/katana.h', 'utf-8')
...
var Generator = function(module) {
  // C structs representing katana types
  this.cTypes = {}
  
  // C functions representing katana function typecasts
  this.functionCasts = {}
  
  // Counts used by this.uniqueName()
  this.uniqueCounts = {}
...
Generator.prototype = {
    ...
    switch(type.type) {
      ...
      case 'function':
        ...
        if (this.cTypes[type]) {
          ...
          var unit = name + '{ void * closure; ' + this.toCType(type.subtypes[0]) + ' (*function) (void *, struct var_t, '  + args.join(', ') + '); };';
  ...
  /**
   * Returns a function cast from one function type to another.
   *
   * @param {Type} from
   * @param {Type} to
   */
  functionCast: function(from, to) {
    if (!this.functionCasts[[from, to]]) {
      var unit = ''

      // Generate a name for the struct that will store the original katana function
      var structName = 'struct ' + this.uniqueName('function_cast_storage')
      
      // Generate a name for the C function that represents the katana function cast
      ...
      // Generate code the struct definition
      unit += structName + ' { ' + this.toCType(from) + ' original; };' 
      
      // Start generating code for the function
      unit += this.toCType(to.returnType()) + ' ' + name + '(void *parent, struct var_t this, ' + args.join(', ') + ') {' 
      
      // Retrieve the storage struct from the `parent` argument taken by katana functions
      unit += structName + ' *storage = parent;'
      
      // Convert each of the arguments to its original type
      ...
      if (!from.returnType().is('void')) {
        // Call original function, grab return value
        ...
        // Convert return value to new type and return
      ...
      // Finish generating code for the function
      unit += '};'
    ...
    return this.functionCasts[[from, to]]
  },
  ...
  generateCast: function(inline, from, to) {
    ...
    if (from.equals(to)) {
      ...
      if (from.is('function')) {
        if (to.is('function')) {
          // Get desired function cast C function
          var functionCast = this.functionCast(from, to)
          
          // Copy original katana function structure to the storage stucture
          ...
          // Create a brand new katana function structure
  ...
  'function literal': function(functionLiteral) {
    ...
    if (functionLiteral.children.length == 2) {
      args = functionLiteral.children[0].children.map(function(arg){
        return 'struct var_t ' + this.rewriteName(arg.meta.info.name)
      }, this)
    } else {
      args = ['void']
    }
    ...
    code.unit += declarationCode.unit + statementListCode.unit
    code.unit +=
      'static ' + returnType + ' ' + name + '( void *parent, struct var_t this,' + args.join(', ') + ') {' +
         declarationCode.block +
         statementListCode.block +
      '};'
