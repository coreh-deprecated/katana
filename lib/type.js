...
Type.prototype.canPromoteTo = function(to) {
  ...
  case 'var':
    ...
    return true
  case 'float':
    // float can accept int, uint and float of a smaller size, or bool
    ...
    break;
  case 'int':
    // int can accept int and uint of a smaller size, or bool
    if (this.is('int', 'uint')) {
      if (to.size() > this.size()) {
        return true
      }
    ...
    break;
  case 'uint':
    // uint can accept uint of a smaller size, or bool
    if (this.is('uint')) {
      if (to.size() > this.size()) {
        return true
      }
