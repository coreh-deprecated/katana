...
Type.prototype.is = function(type) {
  return this.type.replace(/[0-9]+/, '') == type
}
...
Type.prototype.canPromoteTo = function(to) {
  ...
  case to.is('float'):
    ...
    if (this.is('int') || this.is('uint') || this.is('float')) {
...
Type.prototype.canCastTo = function(to) {
  ...
  if ((this.is('int') || this.is('uint') || this.is('float') || this.is('bool')) &&
      (to.is('int') || to.is('uint') || to.is('float') || this.is('bool'))) {
    return true
  }
  return false
}
...
Type.prototype.isLogical = function() {
  return this.is('float') || this.is('int') || this.is('uint') || this.is('bool') || this.is('pointer') || this.is('var')
}
...
Type.prototype.isNumeric = function() {
  return this.is('float') || this.is('int') || this.is('uint') || this.is('bool') || this.is('var')
}
...
Type.prototype.isIntegral = function() {
  return this.is('int') || this.is('uint') || this.is('var') || this.is('bool')
}
